...
export default function EditorPage() {
  ...
  const {
    status, output, error, time, memory,
    setRunning, setResult, setError,
  } = useExecutionStore()
  ...
  try {
    ...
    const { stdout, stderr, status, time, memory } = data.data

    setResult({
      output: stdout || data.data.output || '',
      error:  stderr || null,
      time,
      memory,
      status,
...
}, [language, code, stdin, setRunning, setResult, setError])
...
<div className="flex-1 flex flex-col md:flex-row overflow-hidden">
  ...
  {/* Right — Output panel desktop */}
  <div className="w-full md:w-[380px] shrink-0 border-t md:border-t-0 md:border-l border-border flex flex-col" style={{ height: '300px' }}>
    <OutputPanel />
  </div>

</div>
 {/* Status bar */}
      <div className="flex items-center gap-4 px-4 h-6 bg-zinc-900 border-t border-border shrink-0 text-xs text-muted-foreground font-mono">
        <span
          className="w-2 h-2 rounded-full"
          style={{ background: currentLanguage.color }}
        />
        <span>{currentLanguage.label}</span>
        <span className="ml-auto">{currentTheme.label}</span>
        <span>·</span>
        <span>{fontSize}px</span>
        <span>·</span>
        <span>UTF-8</span>
        <span>·</span>
        <span className={status === 'running' ? 'text-yellow-400' : status === 'success' ? 'text-emerald-400' : 'text-muted-foreground'}>
          {status === 'running' ? '⟳ Running...' : status === 'success' ? '✓ Done' : '● Ready'}
        </span>
      </div>
